...
async def fetch_loop(fn, period, max_errors):
    ...
    while not shutdown:
        try:
            ...
            num_errors_row += 1
            logger.error(
                "Error (num %d, max %d) reading BMS: %s", num_errors_row, max_errors, e
            )
        ...
        if timeout:
            ...
            if pdt > timeout:
                if mqqt_last_publish_time():
                    logger.error(
                        "MQTT message publish timeout (last %.0fs ago), exit", pdt
                    )
                else:
                    logger.error(
                        "MQTT never published a message after %.0fs, exit", timeout
                    )
                shutdown = True
                break
...
async def main(user_config):
    ...
    def name2addr(name: str):
        return next(
            (d.address for d in devices if (d.name or "").strip() == name.strip()), name
        )
        ...
        if isinstance(bms, VirtualGroupBms):
            ...
            for member_ref in bms.get_member_refs():
                if member_ref not in bms_by_name:
                    raise Exception(
                        "unknown bms %s in group %s" % (member_ref, group_bms)
                    )
                member_name = bms_by_name[member_ref].name
                if member_name in groups_by_bms:
                    raise Exception(
                        "can't add bms %s to multiple groups %s %s",
                        member_name,
                        groups_by_bms[member_name],
                        group_bms,
                    )
    ...
    if port_idx > 0:
        user_config.mqtt_port = user_config.get(
            "mqtt_port", int(user_config.mqtt_broker[(port_idx + 1) :])
        )
    ...
    try:
        mqtt_client.connect(
            user_config.mqtt_broker, port=user_config.get("mqtt_port", 1883)
        )
    ...
    expire_values_after = float(
        user_config.get("expire_values_after", MIN_VALUE_EXPIRY)
    )
    ...
    sampler_list = [
        BmsSampler(
            ...
            dt_max_seconds=max(4.0, sample_period * 2),
            expire_after_seconds=max(
                expire_values_after,
                int(sample_period * 2 + 0.5),
                int(publish_period * 2 + 0.5),
            ),
            invert_current=ic,
            meter_state=meter_states.get(bms.name),
            publish_period=publish_period,
            algorithms=dev_args[bms.name].get("algorithm")
            and dev_args[bms.name].get("algorithm", "").split(";"),
            current_correction_factor=dev_args[bms.name].get(
    ...
    sampler_list = sorted(
        sampler_list, key=lambda s: isinstance(s.bms, VirtualGroupBms)
    )

    parallel_fetch = user_config.get("concurrent_sampling", False)

    logger.info(
        "Fetching %d BMS + %d others %s, period=%.2fs, keep_alive=%s",
        len(sampler_list),
        len(extra_tasks),
        "concurrently" if parallel_fetch else "serially",
        sample_period,
        user_config.get("keep_alive", False),
    )
    ...
    if parallel_fetch:
        ...
        loops = [
            asyncio.create_task(
                fetch_loop(fn, period=sample_period, max_errors=max_errors)
            )
            for fn in tasks
        ]
        ...
        async def fn():
            if parallel_fetch:
                ...
                if exceptions:
                    logger.error(
                        "%d exceptions occurred fetching BMSs", len(exceptions)
                    )
                    raise exceptions[0]
...
def on_exit(*args, **kwargs):
    global shutdown
    logger.info(
        "exit signal handler... %s, %s, shutdown already %s", args, kwargs, shutdown
    )
    shutdown = True
...
parser = argparse.ArgumentParser(
    "batmon",
    description="Monitor various types of BMS system",
