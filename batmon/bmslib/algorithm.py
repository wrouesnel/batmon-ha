...
from batmon.bmslib.util import dict_to_short_string, get_logger
...
class SocState:
    ...
    def __str__(self):
        return (
            f"SocState(chg={self.charging}, t_calib={int(self.last_calibration_time)})"
        )
...
class Soc(BaseAlgorithm):
    ...
    def update(self, sample: BmsSample) -> UpdateResult:
        ...
        if self.args.calibration_interval:
            need_calibration = (
                sample.timestamp - self.state.last_calibration_time
                > self.args.calibration_interval
            )
            if need_calibration:
                pass
...
def create_algorithm(repr: Union[dict, str], bms_name=None) -> BaseAlgorithm:
    ...
    algo = classes[name](
        ...
        state=SocState(**state)
        if state
        else SocState(charging=True, last_calibration_time=time.time()),
    )

    if state:
        logger.info(
            "Restored %s algo [args=%s] state %s",
            name,
            algo.args,
            dict_to_short_string(state),
        )
    else:
