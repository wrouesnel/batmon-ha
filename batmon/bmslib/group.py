...
class BmsGroup:
    ...
    def update(self, bms: BtBms, sample: BmsSample):
        assert bms.name in self.bms_names, "bms %s not in group %s" % (
            bms.name,
            self.bms_names,
        )
...
class VirtualGroupBms:
    ...
    def debug_data(self):
        return "missing %s" % (
            set(self.group.bms_names) - set(self.group.samples.keys())
        )
    ...
    async def connect(self):
        ...
        raise GroupNotReady(
            "group %s waiting for member data %s" % (self.name, self.debug_data())
        )
    ...
    async def set_switch(self, switch: str, state: bool):
        for bms in self.members:
            try:
                ...
                self.logger.error(
                    "Group %s failed to set %s switch for %s: %s",
                    self.name,
                    switch,
                    bms.name,
                    ex,
