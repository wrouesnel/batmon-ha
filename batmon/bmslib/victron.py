...
VICTRON_CHARACTERISTICS = {
    "charge": dict(  # consumend Ah
        ...
        unit="Ah",
        func=lambda b: int.from_bytes(b, "little", signed=True) * 0.1,
    ...
    "voltage": dict(
        ...
        unit="V",
        func=lambda b: int.from_bytes(b, "little", signed=True) * 0.01,
    ),
    "current": dict(
        uuid="6597ed8c-4bda-4c1e-af4b-551c4cf74769",
        unit="A",
        func=lambda b: -int.from_bytes(b, "little", signed=True) * 0.001,
    ),
    "soc": dict(
        uuid="65970fff-4bda-4c1e-af4b-551c4cf74769",
        unit="%",
        func=lambda b: int.from_bytes(b, "little", signed=False) * 0.01,
        na_bytes=b"\xff\xff",
    ),
}
...
class SmartShuntBt(BtBms):
    ...
    async def _keep_alive_loop(self):
        ...
        while True:
            ...
            await self.client.write_gatt_char(
                "6597ffff-4bda-4c1e-af4b-551c4cf74769", data, response=False
            )
    ...
    async def connect(self, timeout=20):
        ...
        for k, char in VICTRON_CHARACTERISTICS.items():
            self._values[k] = parse_value(
                await self.client.read_gatt_char(char["uuid"]), char
            )
            await self.client.start_notify(
                char["uuid"], partial(self._handle_notification, k)
            )
